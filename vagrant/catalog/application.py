...
def catalog():
    ...
    return render_template('catalog.html', category_items = category_items, items = items, item_size = item_size,
                           category_name = "all", user_name = user_name)
...
def displayCatalogItemDetail(category_name, item_name):
    ...
    print str(item.id) + "." + item.filetype
    if item.filetype != "none":
        
        image = url_for('uploaded_file', image=str(item.id) + "." + item.filetype)
    else:
        image = ""
    
    return render_template('catalogItem.html', item = item, category_name = category_name,
            user_edit_access = user_edit_access,image = image )
...
def editCatalogItem(category_name, item_name):
	...
	session.commit()
        ...
        if file and allowed_file(file.filename):
            ...
            file_type = "none"
        if file_type != "none":
            # delete the old file and replace it with the new file on the server
...
def addCatalogItem():
    ...
    if request.method == 'POST':
        ...
        if item_size == 0:
            ...
            filename = secure_filename(file.filename)
            if file and allowed_file(file.filename):
                # get the file type to store and for naming the file on the server
                file_type = filename.rsplit('.', 1)[1]
            ...
            newItem = CatalogItem(name = request.form['name'],
                ...
                category_id = request.form['category_id'], user_id =
                login_session.get('appuserid'), filetype = file_type)
            session.add(newItem)
            session.commit()
            flash("New Catalog Item has been added - %s" % request.form['name'])
            # If a file is attached, name the file with the item number and the original extention
            if file_type != "none":
                item = session.query(CatalogItem).filter_by(name = request.form['name']).one()
                save_name = str(item.id) + "." + filename.rsplit('.', 1)[1]
                file.save(os.path.join(app.config['UPLOAD_FOLDER'], save_name))
            return redirect(url_for('catalog'))
        else:
